...
- **Improve one-sim vs two-sim comparison in `sandbox_Espen.ipynb`:** The current comparison is not budget-equal (two-sim uses 2×B_CAL paths, one-sim uses B_CAL). The planned fair design: both methods share the same step-1 batch (B_CAL paths, drawn once per repeat), one-sim reuses it for step 2 while two-sim draws a fresh B_CAL-path batch for step 2. Verification paths should also be shared between the two methods per repeat. This removes step-1 randomness as a confound and isolates the step-2 in-sample bias as the only difference between the box plots.
