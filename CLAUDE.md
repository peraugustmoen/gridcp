...
- `rng=None` → deterministic (fixed internal seed); `rng=int` → deterministic from that seed; `rng=Generator` → seeded from generator's original `SeedSequence`, caller's state not advanced
